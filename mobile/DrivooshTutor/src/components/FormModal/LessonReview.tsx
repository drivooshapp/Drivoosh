...
//                         <Text style={styles.sectionTitle}>משוב והערות על השיעור</Text>
//                         <TextInput
...
//     checkboxRow: { flexDirection: 'row', justifyContent: 'flex-end', alignItems: 'center', paddingVertical: 8, marginBottom: 10 },
...
export default function LessonReview({
    ...
    // אכיפה שדירוג התחלתי לא יהיה פחות מ-1
    const [rating, setRating] = useState(initialRating < 1 ? 1 : initialRating);
    const [notes, setNotes] = useState(initialNotes);
    const [isChecked, setIsChecked] = useState(initialChecked);
    
    // סטייט חדש לניהול הודעת שגיאה אם המורה מנסה לשמור בלי לסמן את התיבה
    const [showError, setShowError] = useState(false);

    useEffect(() => {
        // בכל פתיחה מחדש, נאתחל את הערכים ונדאג שהדירוג המינימלי הוא 1
    ...
    const handleSave = () => {
        // אכיפה קשיחה: התיבה חייבת להיות מסומנת
        if (!isChecked) {
            setShowError(true);
            return; // מונע את השמירה ומחזיר את המורה לתקן
        }

        // אם הכל תקין, נשמור (הדירוג מובטח להיות לפחות 1 בזכות הסטייט)
        setShowError(false);
        onSave(rating, notes, isChecked);
        onClose();
    };
    ...
    const handleCheckboxToggle = () => {
        ...
        if (newValue) {
            setShowError(false); // העלמת השגיאה ברגע שהוא מסמן
        }
    };
            ...
            >
                <View style={styles.modalContent}>
                    ...
                    <ScrollView style={styles.modalBody} keyboardShouldPersistTaps="handled">
                        ...
                        {/* תצוגת שגיאה במידה והתיבה לא מסומנת */}
                        {showError && (
                        ...
                        <TouchableOpacity
                            style={[
                                styles.checkboxRow, 
                                showError && styles.checkboxRowError // הדגשה אדומה קלה במקרה של שגיאה
                            ]}
                        ...
                        <View style={styles.starsContainer}>
                            {[1, 2, 3].map((num) => (
                                <TouchableOpacity
                                    key={num}
                                    onPress={() => setRating(num)} // הדירוג המינימלי הוא תמיד 1 לפחות
                                    style={styles.starTouch}
                                >
                                    <Ionicons
                                        name={num <= rating ? "star" : "star-outline"}
